	...
	class change_profile extends load{
		...
		function __construct(){
			...
			if(($this->check_email() == true) and (!empty($this->post("email"))))
				$response = array("adminpanel/profileset", "Email tidak valid");

			elseif(($this->verifikasi() == false) and (!empty($this->post("email"))))
				$response = array("adminpanel/profileset", "Kode verifikasi salah");

			elseif(($this->check_oldpassword() == false) and (!empty($this->post("oldpassword"))))
				$response = array("adminpanel/profileset", "Password lama salah");

			elseif(($this->check_match_password() == false) and (!empty($this->post("oldpassword"))))
				$response = array("adminpanel/profileset", "Password baru tidak cocok");

			else{

				$response = array("adminpanel/profileset", "Berhasil");
				$this->input_data();
		...
		function input_data(){
			...
			save_activity("Melakukan perubahan profile pribadi");
