...
<form method="get" action="<?php echo HomeUrl()?>/adminpanel/request_refund" style="float:right">
...
<div class="big-panel-box" style="margin-top:20px;">
	...
	<div class="list">
		<table width="100%">
			...
			<?php
			$paging = $this->pagination(false,"db_wallet","request_refund");
			...
			while($show = $query->fetch()){ 
				...
				if($show['type'] == 0){
					...
					if($show['status'] == 1){
						$saldo = $saldo['saldo'] + $show['saldo'];
						$status = "<span style='color:#1093ad;font-family:Segoe UI Bold'>[ Dana Telah Dikembalikan ]</span>";
					}else{
						$saldo = $saldo['saldo'];
						$status = "";
					}
			...
			<tr>
				...
				<td style="width: 150px;">
					...
					<p style="font-size: 16px;margin-top:-10px;font-family: Segoe UI Bold;color:<?php echo $color?>">Rp <?php echo str_replace("-",null,number_format($show['saldo']))?></p>
				</td>
				<td style="width: 150px;">
					<a href="<?php echo HomeUrl()?>/adminpanel/refund_status?id=<?php echo $show['tf_id']?>" style="color:orangered">Validasi Permintaan</a>
				</td>
			</tr>
...
<div style="margin-top:0px;float: right;width: 100%">
	...
	<div style="float: right;margin-top:-5px;">
		<?php echo $this->pagination(true,"db_wallet","request_refund") ?>
	</div>
