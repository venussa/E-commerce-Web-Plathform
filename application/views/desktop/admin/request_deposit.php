...
<form method="get" action="<?php echo HomeUrl()?>/adminpanel/request_deposit" style="float:right">
		...
		<table width="100%">
			...
			while($show = $query->fetch()){ 
				...
				if($show['type'] == 0){
					...
					if($show['status'] == 1){
						$saldo = $saldo['saldo'] + $show['saldo'];
						$status = "<span style='color:#1093ad;font-family:Segoe UI Bold'>[ Dana Terdeposit ]</span>";
					}else{
						$saldo = $saldo['saldo'];
						$status = "";
					}
