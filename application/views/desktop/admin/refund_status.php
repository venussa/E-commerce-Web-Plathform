...
$query = $this->db()->query("SELECT * FROM db_wallet WHERE tf_id='".urldecode($this->get("id",["'"]))."' and type='1'");

if($query->rowCount() == 0){

	header("location:".HomeUrl()."/adminpanel/request_refund");
	exit;
}
...
<form method="POST" action="<?php echo HomeUrl()?>/adminpanel/verif_refund?id=<?php echo urldecode($this->get("id",["'"]))?>" enctype="multipart/form-data">
<table width="100%" style="">
	
	<tr>
		<td class="tr" style="width: 200px;">
			<p class="t1">Informasi Akun</p>
			<p class="t2">* Nama akun yang meminta penarikan saldo</p>
		</td>
		<td>
	...
	<tr>
		...
		<td><input type="text" class="form-input" disabled required value="<?php echo $show['card_name']?>"></td>
	</tr>
	...
	<tr>
		...
		<td><input type="text" class="form-input" disabled required value="<?php echo $show['bank_name']?>"></td>
	</tr>
	...
	<tr>
		...
		<td><input type="text" class="form-input" disabled required value="<?php echo $show['rekening_number']?>"></td>
	</tr>
	<tr>
		<td class="tr" style="width: 200px;">
		...
		<td><input type="text" class="form-input" disabled required value="Rp <?php echo str_replace("-",null,number_format($show['saldo']))?>"></td>
	</tr>
	<tr>
		<td></td>
		<td>
			<label class="cb-container">Check bila anda sudah mengembalikan saldo kepada customer
