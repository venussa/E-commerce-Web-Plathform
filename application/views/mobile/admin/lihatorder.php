...
<form method="POST" action="<?php echo HomeUrl()?>/adminpanel/verifikasi_order?id=<?php echo $show['sorting_id']?>" enctype="multipart/form-data">
	<table width="100%" style="">
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style=""><?php echo date("d/m/Y H:i",$log_order['time'])?></p>
		...
		<tr>
			...
			<p>#<?php echo $show["invoice_id"]?></p></td>
		</tr>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style="font-weight:600;color:#09f;cursor:pointer;" onClick="window.location='<?php echo HomeUrl()?>/adminpanel/product?q=<?php echo $product['product_id']?>';"><?php echo $product['title']?></p>
				<p style="font-size:11px;"><span style="font-weight:600;">Product Id : </span><?php echo $product['product_id']?></p>
				<p style="font-size:11px;color: orangered"><span style="font-weight:600;color:#000">Price : </span>Rp <?php echo number_format($show['price'])?></p>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style="font-size:11px;"><span style="font-weight:600;">recipient's name : </span><?php echo $show['nama_penerima']?></p>
				<p style="font-size:11px;"><span style="font-weight:600;">Phone Number : </span><?php echo $show['phone_number']?></p>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style="font-weight:600;margin-left:60px;margin-top:0px;"><?php echo $delivery_service['company_name']?></p>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p><span style="font-weight:600;font-size:13px;color:#000;">Bayar penuh dengan saldo Dompet Digital</span><br>
					<span style="font-size:13px;color:#000;">ID Transaksi : <?php echo $wallet['tf_id']?></span>
				</p>
			</td>
		</tr>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p><span style="font-weight:600;font-size:13px;color:#000;">Bayar sebagian dengan saldo Dompet Digital</span><br>
					<span style="font-size:13px;color:#000;">ID Transaksi : <?php echo $wallet['tf_id']?></span><br>
					<span style="font-size:13px;color:orangered;">Rp <?php echo number_format(($wallet['saldo']))?></span><br>
					Sisa yang belum dibayar : <span style="font-size:13px;color:orangered;">Rp <?php echo number_format(($pay_method - $wallet['saldo']))?></span>
				</p>
			</td>
		</tr>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style="font-weight:600;color:#09f;cursor: pointer;"><?php echo $bank['bank_name']?> <span style="font-weight:600;font-size:13px;color:#666;">(<?php echo $bank['bank_name']?>)</span></p>
			</td>
		</tr>
		<?php  }else{ ?>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<div style="background: #f5f5f5;border-radius: 3px;border:1px #ddd solid;">
					<p style="font-size:11px;border-bottom:1px #ddd solid;padding:10px;"><span style="font-weight:600;">Total Order : </span><?php echo $show['total_order']?></p>
					<p style="font-size:11px;border-bottom:1px #ddd solid;padding:10px;"><span style="font-weight:600;">Product Price : </span>Rp <?php echo number_format($show['price'])?></p>
					<p style="font-size:11px;border-bottom:1px #ddd solid;padding:10px;"><span style="font-weight:600;">Total Price : </span>Rp <?php echo $total_price?></p>
					<p style="font-size:11px;border-bottom:1px #ddd solid;padding:10px;"><span style="font-weight:600;">Shipping Cost : </span>
					...
					<p style="font-size:11px;padding:10px;color:orangered;"><span style="font-weight:600;">Total Cost : </span>
						<?php if(($product['transaction_scheme'] > 0) and ($order_pending == 0)) {
						echo "Belum Ada";
						}else{ ?>
			...
			while($show_log = $log_order->fetch()){ 
				...
				<p style="margin-bottom: 50px;margin-left: 20px;width: 50%">

				<button class="circle-progres-big" style="border: 2px <?php echo $order_type['color']?> solid;color:<?php echo $order_type['color']?>;position: absolute;"><b><?php echo $order_type['type']?></b></button>


				<?php
				if($sum < $log_order->rowCount()) { ?>
				<button class="vertical-line" style="margin-top:30px;height:60px;"></button>
			<?php } ?>

					<p style="left:60px;position: relative;top:-40px;width: 80%">[<?php echo date("d/m/Y H:i",$show_log['time'])?>]<br><?php echo $order_type['msg']?></p>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style="color:#666;font-style: italic;">Komplain di tangani oleh <a style="font-weight:600;color:#09f" href="<?php echo HomeUrl()?>/adminpanel/users?q=<?php echo $handle['username']?>"><?php echo ucwords($handle['first_name']." ".$handle['last_name'])?></a> ( Anda )</p>
				...
				<p style="color:#666;font-style: italic;">Komplain di tangani oleh <a style="font-weight:600;color:#09f" href="<?php echo HomeUrl()?>/adminpanel/users?q=<?php echo $handle['username']?>"><?php echo ucwords($handle['first_name']." ".$handle['last_name'])?></a></p>
				<p><a href="<?php echo HomeUrl()?>/adminpanel/message?chat_to=<?php echo $user['username']?>&action=complain&id=<?php echo $show['sorting_id']?>#<?php echo $user['username']?>"><button class="btn-white" type="button">Lihat Aktivitas</button></a></p>
				...
				<p style="color:#666;font-style: italic;">Komplain di tangani oleh <a style="font-weight:600;color:#09f" href="<?php echo HomeUrl()?>/adminpanel/users?q=<?php echo $handle['username']?>"><?php echo ucwords($handle['first_name']." ".$handle['last_name'])?></a></p>
				<p style="color:green;font-style: italic;font-weight:600;">Komplain Selesai</p>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style=""><span style="font-weight:600;">Bank Name : </span><?php echo $show['your_bank_name']?></p>
				<p style=""><span style="font-weight:600;">Rekening Number : </span><?php echo $show['your_rekening_number']?></p>
				<p style=""><span style="font-weight:600;">A/N : </span><?php echo  $show['your_atas_nama']?></p>
	...
	<?php } ?>
			...
			<?php if((isset($show_prov)) and (!isset($show_verif)) and (($cancel_order->rowCount() == 0) or ($show_cancel_order['status'] == 2)) ) { ?> 
				...
				<tr>
					<td class="tr" style="width: 200px;">
						<button class="btn-white">Verifikasi</button></td>
				</tr>
			...
			<?php if((isset($show_verif)) and (!isset($show_item_send)) and (($cancel_order->rowCount() == 0) or ($show_cancel_order['status'] == 2)) ) { ?>
				...
				<tr>
					<td class="tr" style="width: 200px;">
						...
						<p style="padding-top:10px;border-top:2px #ddd dashed;font-weight:600;">Nomor Resi Jasa Expedisi</p>
			...
			<?php if((isset($complain)) and ((isset($resolve)) or (isset($refund)) )){ ?>
					...
					<td class="tr" style="width: 200px;">
						...
						<p style="padding-top:10px;border-top:2px #ddd dashed;font-weight:600;">Catatan <span style="color:#666;font-size:11px;"> ( * Catatan hasil negosiasi dengan customer )</span></p>
		...
		<tr>
			<td class="tr" style="width: 200px;">
				...
				<p style="padding-top:10px;border-top:2px #ddd dashed;font-weight:600;">Alasan Pembatalan</p>
