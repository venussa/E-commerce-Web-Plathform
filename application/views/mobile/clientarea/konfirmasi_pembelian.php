...
box-shadow: 1px 3px 6px 1px rgba(0,0,0,0.2);padding: 10px;border-radius: 5px;">Proses Checkout</p>
...
<p style="font-weight:600;color:#434343;font-size: 17px">Alamat Pengiriman</p>
...
<?php $no = 1; while($show = $address->fetch()){?>
	...
	<?php if($show['id'] == $this->post("address_id")){ ?>
		...
		<label class="cb-container" style="font-size: 12px;"> <?php echo $show['address']?>, <?php echo $show['district']?>, <?php echo $show['state']?>, <?php echo $show['province']?>, <?php echo $show['zip_code']?> ( Tlp : <?php echo $show['phone_number']?> )
...
<p align="right"><a href="<?php echo HomeUrl()?>/clientarea/alamat_pengiriman" style="color:#fe4c50;float: right;font-size: 14px;margin-right:10px;">Tambah Alamat</a></p>

<p style="font-weight:600;border-top:2px #ddd dashed;padding-top: 10px;margin-top: 50px;color:#434343;font-size: 17px">Informasi Barang</p>
...
<table width="100%">
	...
	<tr>
		...
		<td style="width: 120px;border-bottom: 2px #ddd dashed;padding-top:10px;" valign="top">
			<img src="<?php echo sourceUrl()."/content/".$picture[0]?>" style="width: 110px;border:1px #ddd solid;">
		</td>
		...
		<td valign="top" style="border-bottom: 2px #ddd dashed;">
			...
			<p style="font-size:14px;margin-top:3px;">
				<a style="font-weight:600;color:#434343;" href="<?php echo $url?>"><?php echo strip_tags($product['title'])?></a></p>
			<p style="color:#666;font-size:14px;margin-top:-15px;font-size: 13px;">1 barang ( <?php echo $product['weight']?> gr) </p>
			<p style="color:orangered;font-weight:600;font-size:14px;margin-top:-10px;"><?php echo $discount_price ?></p>
			
		</td>
	</tr>
	<tr>
		<td colspan="2">
			<p><textarea style="border:1px #ccc solid; width: 94%;padding: 10px;"  placeholder="Note kepada penjual" onKeyup="return write_note(this)"></textarea></p>
		</td>
	</tr>

</table>

<p style="font-weight:600;padding-top: 10px;padding-bottom: 10px;margin-top: 10px;color:#434343;font-size: 17px">Jasa & Durasi Pengiriman</p>
...
<?php foreach($delivery_service as $key => $val){
	...
	?>
	<div style="padding:5px;padding-top:8px;border-top: 1px #ccc solid;">
		...
		<p style="font-weight:600;margin-left:99px;margin-top:2px;"><?php echo $val->company_name?></p>
		...
		<p style="margin-left: 99px;color:orangered;font-size:12px;margin-top:-9px;font-weight:600">Rp <?php echo number_format($val->price)?></p>
		<?php } ?>
...
<?php if($product['transaction_scheme'] < 1){ ?>
<div style="width: 97.5%;background: #f5f5f5;padding-left:7px;border:1px #ddd solid;padding-bottom:15px;border-radius: 5px;margin-top: 30px;">
<p style="font-weight:600;color:#434343;">Ringkasan Belanja</p>
...
<table width="100%">
	...
	<tr>
		...
		<td style="border-bottom:1px #ddd solid;padding:5px;color: #434343;font-size: 13px;font-weight:600;text-align: right;">Rp <?php echo number_format(($this->post("jumlah") * $product_price))?></td>
	...
	<tr>
		...
		<td style="border-bottom:1px #ddd solid;padding:5px;color: #434343;font-size: 13px;font-weight:600;text-align: right;">Rp <?php echo number_format($cost)?></td>
	...
	<tr>
		...
		<td style="border-bottom:1px #ddd solid;padding:5px;color: orangered;font-size: 13px;font-weight:600;text-align: right;">Rp <?php echo number_format(($this->post("jumlah") * $product_price) + $cost)?></td>
...
<div style="width: 100%;margin-top: 40px;border-top: 1px #ccc solid;">
<p style="font-weight:600;padding-top: 10px;margin-top: 20px;color:#434343;font-size: 17px">Bayar Menggunakan</p>
...
<form method="POST" action="<?php echo HomeUrl()?>/clientarea/finish_order">
	...
	<?php 
		...
		?>
			...
			<img class="picture-bank" src="<?php echo sourceUrl()?>/bank/<?php echo $show['icon']?>" style="width:70px;height:40px;border-radius: 6px;border:2px orangered solid;padding:1px;cursor: pointer;margin-right: 10px" data="<?php echo $show['id']?>" onClick="return select_bank(this)">

			<?php }else{ ?>

			<img class="picture-bank" src="<?php echo sourceUrl()?>/bank/<?php echo $show['icon']?>" style="width:70px;height:40px;border-radius: 6px;border:1px #ccc solid;padding: 1px;cursor: pointer;margin-right: 10px" data="<?php echo $show['id']?>" onClick="return select_bank(this)">
	...
	<p style="border-bottom: 1px #ccc solid;border-top: 1px #ccc solid;padding-bottom: 10px;padding-top: 10px;color:#666;margin-bottom: 10px;">
	...
	<p style="font-size:11px;color:#666;padding: 10px;">
	...
	<?php } ?>
	<button class="btn-white" style="padding:10px;margin-top: 10px;background: #fe4c50;color:#fff;border:1px #fe4c50 solid;border-radius: 5px;width:100%">Selesaikan Order</button>
