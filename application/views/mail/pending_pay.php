<div style="width: 600px;border:1px #ddd solid;padding:10px;border-radius: 5px">
	...
	<h3 style="font-family: Arial">Mohon Segera Selesaikan Pembayaran Anda</h3>
	<p style="color:#666;font-family: Arial;font-size: 13px;">Checkout berhasil pada tanggal <?php echo date("d M Y, H:i")?> WIB. <?php if(isset($hide_msg)) echo $hide_msg; ?></p>
	...
	<table width="100%" style="background: #f5f5f5;padding: 10px;">
		<tr>
			<td valign="top" style="width: 300px;"><p style="font-weight: 600">Total Pembayaran</p>
				...
				<?php if(($this->post("wallet") == "1") or (isset($new_total))) { ?>
				<p style="font-size: 14px;color:#666">Rp <?php echo number_format($new_total)?></p>
				<p style="font-size: 11px;color:#666;margin-top:-10px">* Sebagian sisanya sudah dibayar dengan saldo dompet digital</p>
				<?php }else{ ?>
				<p style="font-size: 14px;color:#666">Rp <?php echo number_format($total)?></p>
				<?php } ?>
			</td>
			<td valign="top"><p style="font-weight: 600">Batas Waktu</p>
				<p style="font-size: 14px;color:#666"><?php echo date("d M Y, H:i", ($order['start_time']+86400))?> WIB</p>
			</td>
		</tr>
		<tr>
			<td valign="top"><p style="font-weight: 600">Metode Pembayaran</p>
				...
				<p style="font-size: 14px;color:#666">Transfer <?php echo $bank['bank_info']?></p>
				<p style="font-size: 12px;color:#666">No. Rek : <?php echo $bank['bank_info']?> (<?php echo $bank['atas_nama']?>)<br>Code Bank : <b><?php echo $bank['code_bank']?></b></p>
			</td>
	...
	<br>
	<br>
	<a target="_blank" href="<?php echo HomeUrl()."/clientarea/konfirmasi_pembayaran"?>?id=<?php echo $order['sorting_id']?>" style="cursor:pointer;"><button style="background: #fe4c50;border:1px #fe4c50 solid;color:#fff;padding:10px;font-family: Arial;border-radius: 5px;cursor: pointer;">Upload Bukti Pembayaran</button></a>
